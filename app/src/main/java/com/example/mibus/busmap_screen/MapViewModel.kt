...
import java.util.*
// TODO:Refactoring BusViewModel
class BusViewModel : ViewModel() {

   private val callback: OnMapReadyCallback = OnMapReadyCallback { googleMap ->
      val nizhniyNovgorod = LatLng(56.3, 44.0)
      googleMap.addMarker(
         MarkerOptions().position(nizhniyNovgorod).title("Marker in Nizhniy-Novgorod")
      )
      val cameraUpdate = CameraUpdateFactory.newLatLngZoom(nizhniyNovgorod, 12F)
   ...
   fun setPointClick(map: GoogleMap) {
      ...
      map.setOnPoiClickListener { point ->
         val pointMarker = map.addMarker(MarkerOptions().position(point.latLng).title(point.name))
      }
   ...
   fun setMapLongClick(map: GoogleMap) {
      map.setOnMapClickListener {
         val snippet = String.format(
            Locale.getDefault(), "Lat: %1$.5f, Lng: %2$.5f", it.longitude, it.latitude
         )
   ...
   fun mapPointList(): LatLng {
      ...
      latLngList = mutableListOf(
         LatLng(56.3, 44.0), LatLng(56.3, 43.0), LatLng(56.3, 42.0)
      )
      latLngList.shuffle()
